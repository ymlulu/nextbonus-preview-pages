(() => {
  ...
  function visibleEntries(){
    const category=FILTER_CATEGORY[ui.filter]||null;
    const query=ui.query.trim().toLowerCase();
    return catalogEntries().filter(({category:itemCategory,product})=>{
  ...
  function replaceLabelForInput(modal,inputId,text){
  ...
  function enhanceInfo(modal){
    const body=modal.querySelector('.modal-body');
    const title=modal.querySelector('.modal-title');
    if(title) title.textContent='补充信息';
    if(body && ui.selectedProduct && !body.querySelector('.nb-selected-product')){
      body.insertAdjacentHTML('afterbegin',`<div class="report nb-selected-product"><h3>${esc(ui.selectedProduct.product.name)}</h3><p>${esc(ui.selectedProduct.product.institution)}</p></div>`);
    }
    const isCard=ui.selectedProduct?.category==='信用卡' || !!modal.querySelector('#add-last4');
  ...
  function enhanceTrack(modal){
    const isCard=ui.selectedProduct?.category==='信用卡';
    const title=modal.querySelector('.modal-title');
    if(title) title.textContent=isCard?'要一起追踪开卡奖励吗？':'要一起追踪开户奖励吗？';
    const yes=modal.querySelector('[data-action="add-track-choice"][data-value="yes"]');
    const no=modal.querySelector('[data-action="add-track-choice"][data-value="no"]');
    if(yes){
      const t=yes.querySelector('.option-title'), s=yes.querySelector('.option-sub');
      if(t) t.textContent=isCard?'追踪这次开卡奖励':'追踪这次开户奖励';
      if(s) s.textContent=isCard?'选择你申请时看到的奖励，之后可以记录进度和截止日期。':'选择你开户时看到的奖励，之后可以记录进度和截止日期。';
    }
    if(no){
      const t=no.querySelector('.option-title'), s=no.querySelector('.option-sub');
      if(t) t.textContent='暂时不追踪';
      if(s) s.textContent='以后也可以从产品详情里添加。';
    }
    const next=modal.querySelector('[data-action="add-track-next"]');
    if(next) next.textContent=no?.classList.contains('selected')?'添加到钱包':'继续';
  }

  function enhanceOffer(modal){
    const isCard=ui.selectedProduct?.category==='信用卡';
    const title=modal.querySelector('.modal-title');
    if(title) title.textContent=isCard?'你申请时看到的是哪个奖励？':'你开户时看到的是哪个奖励？';
    modal.querySelectorAll('.option-tag').forEach(node=>node.remove());
    const manual=modal.querySelector('[data-action="add-offer-choice"][data-id="manual"]');
    if(manual){
      const t=manual.querySelector('.option-title'), s=manual.querySelector('.option-sub');
      if(t) t.textContent='都不是，手动填写';
      if(s) s.textContent='按你实际申请或开户时看到的奖励填写。';
    }
    const next=modal.querySelector('[data-action="add-offer-next"]');
    if(next){
      const selected=modal.querySelector('[data-action="add-offer-choice"].selected');
      next.textContent=selected && selected.dataset.id!=='manual'?'添加到钱包':'继续';
    }
  }

  function enhanceManual(modal){
    const title=modal.querySelector('.modal-title');
    if(title) title.textContent='填写你的奖励';
    const reward=modal.querySelector('#add-reward');
    const rewardLabel=reward?.closest('.form-group')?.querySelector('.label');
    if(rewardLabel) rewardLabel.textContent='奖励内容';
    if(reward) reward.setAttribute('placeholder','例如 175,000 MR');
    const cards=[...modal.querySelectorAll('.task-card')];
    const container=cards[0]?.parentElement;
    const sectionLabel=container?.querySelector(':scope > .label');
    if(sectionLabel) sectionLabel.textContent='完成条件';
    cards.forEach((card,index)=>{
      const heading=card.querySelector('.task-head span');
      if(heading) heading.textContent=`完成条件 ${index+1}`;
      const row=card.querySelector('.date-field-row');
      if(row && !card.querySelector('.nb-due-label')) row.insertAdjacentHTML('beforebegin','<label class="label nb-due-label">截止日期</label>');
    });
    const add=modal.querySelector('[data-action="add-task"]');
    if(add) add.textContent='＋ 添加条件';
    const submit=modal.querySelector('[data-action="add-manual-submit"]');
    if(submit) submit.textContent='添加到钱包';
  }

  function enhanceMembership(modal){
    const title=modal.querySelector('.modal-title');
    if(title) title.textContent='确认添加';
  ...
  function enhanceSuccess(modal){
    const title=modal.querySelector('.modal-title');
    const success=modal.querySelector('.success');
    if(title) title.textContent='已添加到钱包';
    const heading=success?.querySelector('h2');
    if(heading) heading.textContent='已添加到钱包';
    let note=success?.querySelector('p');
    if(ui.tracking){
      if(!note && heading){
        note=document.createElement('p');
        heading.insertAdjacentElement('afterend',note);
      }
      if(note) note.textContent='奖励也已经加入提醒。';
    }else if(note){
      note.remove();
    }
    const view=modal.querySelector('[data-action="add-view-product"]');
    const another=modal.querySelector('[data-action="add-another"]');
    if(view) view.textContent='查看详情';
    if(another) another.textContent='继续添加';
  }

  function detectStage(modal){
    if(modal.dataset.nbStage==='selector') return 'selector';
    if(modal.querySelector('.category-grid') || modal.querySelector('#add-search')) return 'selector';
    if(modal.querySelector('#add-last4') || modal.querySelector('#add-nickname')) return 'info';
    if(modal.querySelector('[data-action="add-track-choice"]')) return 'track';
    if(modal.querySelector('[data-action="add-offer-choice"]')) return 'offer';
    if(modal.querySelector('#add-reward')) return 'manual';
    if(modal.querySelector('[data-action="add-view-product"]')) return 'success';
    if((modal.querySelector('.modal-title')?.textContent||'').trim()==='确认添加' && modal.querySelector('.report')) return 'membership';
    return 'other';
  }
  ...
  function enhanceModal(){
    ...
    if(!modal) return;
    setProgressLabels(modal);
    const stage=detectStage(modal);
    modal.dataset.nbStage=stage;
    modal.classList.toggle('nb-unified-selector',stage==='selector');
    if(stage==='selector') renderUnifiedSelector(modal);
    else if(stage==='info') enhanceInfo(modal);
    else if(stage==='track') enhanceTrack(modal);
    else if(stage==='offer') enhanceOffer(modal);
    else if(stage==='manual') enhanceManual(modal);
    else if(stage==='membership') enhanceMembership(modal);
    else if(stage==='success') enhanceSuccess(modal);
    modal.classList.add('nb-add-product-ready');
  }
  ...
  function selectUnifiedProduct(category,id){
    const entry=catalogEntries().find(item=>item.category===category && item.product.id===id);
    if(!entry) return;
  ...
  function showExitConfirm(){
    if(document.querySelector('.nb-add-exit-confirm')) return;
    const overlay=document.createElement('div');
    overlay.className='modal-backdrop nb-add-exit-confirm';
    overlay.innerHTML='<div class="modal"><div class="modal-head"><div class="modal-title">退出添加？</div></div><div class="modal-body"><p class="confirm-copy">已填写的内容不会保存。</p></div><div class="modal-foot"><button class="btn secondary" type="button" data-nb-exit-continue>继续添加</button><button class="btn danger" type="button" data-nb-exit-confirm>退出</button></div></div>';
    document.body.appendChild(overlay);
  }

  function closeWithoutPrompt(){
    document.querySelector('.nb-add-exit-confirm')?.remove();
    triggerAppAction('add-discard');
  }
  ...
  document.addEventListener('click',event=>{
    ...
    const unifiedProduct=target.closest?.('[data-nb-add-product-id]');
    if(unifiedProduct){
      event.preventDefault();
      event.stopImmediatePropagation();
      selectUnifiedProduct(unifiedProduct.dataset.nbAddCategory,unifiedProduct.dataset.nbAddProductId);
      return;
    }
    const filter=target.closest?.('[data-nb-add-filter]');
    if(filter){
      event.preventDefault();
      event.stopImmediatePropagation();
    ...
    if(target.closest?.('[data-nb-clear-search]')){
      event.preventDefault();
      event.stopImmediatePropagation();
      ui.query='';
      const modal=root.querySelector('.add-product-modal');
    ...
    if(target.closest?.('[data-nb-exit-confirm]')){
      event.preventDefault();
      event.stopImmediatePropagation();
      closeWithoutPrompt();
      return;
    ...
    if(!action) return;
    if(action.dataset.action==='open-add-product') resetUi();
    if(action.dataset.action==='add-another') resetUi();
    ...
    if(action.dataset.action==='add-close' && modal){
      const stage=modal.dataset.nbStage||detectStage(modal);
      const progressed=stage!=='selector' || !!ui.selectedProduct;
      if(progressed && stage!=='success'){
        event.preventDefault();
        event.stopImmediatePropagation();
        showExitConfirm();
        return;
    ...
    if(action.dataset.action==='add-back' && modal?.classList.contains('nb-unified-selector')){
      event.preventDefault();
      event.stopImmediatePropagation();
      closeWithoutPrompt();
    }
  ...
  document.addEventListener('input',event=>{
    ...
    if(modal){
      ...
      const input=modal.querySelector('#nb-add-search');
      if(input){
        input.focus();
