(() => {
  ...
  function renderModal(ctx){
    ...
    const m=state.modal;
    if(m.type==='apply-risk') return `<div class="modal-backdrop"><div class="modal"><div class="modal-head"><div class="modal-title">确认继续申请</div><button class="close-btn nb-icon-button nb-icon-button--round nb-icon-button--soft" data-action="modal-close" aria-label="关闭">${window.NextBonusIcons.svg('close')}</button></div><div class="modal-body"><p class="confirm-copy">${esc(m.copy||'你可能无法获得当前开卡奖励。仍要继续申请吗？')}</p><p class="muted">这只是风险确认，不替你强制拦截申请。</p></div><div class="modal-foot"><button class="btn secondary" data-action="modal-close">返回</button><button class="btn primary" data-action="apply-confirm">继续申请</button></div></div></div>`;
    if(m.type==='simple') return `<div class="modal-backdrop"><div class="modal"><div class="modal-head"><div class="modal-title">${esc(m.title)}</div><button class="close-btn nb-icon-button nb-icon-button--round nb-icon-button--soft" data-action="modal-close" aria-label="关闭">${window.NextBonusIcons.svg('close')}</button></div><div class="modal-body"><p class="confirm-copy">${esc(m.copy)}</p>${m.detail?`<p class="muted">${esc(m.detail)}</p>`:''}</div><div class="modal-foot"><span></span><button class="btn primary" data-action="modal-close">知道了</button></div></div></div>`;
    if(m.type==='remove-product-confirm') return `<div class="modal-backdrop"><div class="modal"><div class="modal-head"><div class="modal-title">从 NextBonus 中移除？</div><button class="close-btn nb-icon-button nb-icon-button--round nb-icon-button--soft" data-action="modal-close" aria-label="关闭">${window.NextBonusIcons.svg('close')}</button></div><div class="modal-body"><p class="confirm-copy">这只会删除误添加的 NextBonus 产品记录，不代表关闭真实账户。</p></div><div class="modal-foot"><button class="btn secondary" data-action="modal-close">取消</button><button class="btn danger" data-action="remove-product-confirm" data-id="${esc(m.productId)}">确认移除</button></div></div></div>`;
