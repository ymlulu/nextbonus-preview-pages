(() => {
  ...
  function choiceWithinRecentMonths(choice,todayIso,months=6){
    if(choice?.kind==='current') return true;
    const choiceMonth=String(choice?.dateLabel||'').match(/^(\d{4})-(\d{2})/);
    const currentMonth=String(todayIso||'').match(/^(\d{4})-(\d{2})/);
    if(!choiceMonth||!currentMonth) return false;
    const choiceIndex=Number(choiceMonth[1])*12+Number(choiceMonth[2])-1;
    const currentIndex=Number(currentMonth[1])*12+Number(currentMonth[2])-1;
    const diff=currentIndex-choiceIndex;
  ...
  function render(ctx){
    ...
    if(f.step === 'category' || f.step === 'product'){
      ...
      const visibleChoices=result.choices
        .filter(choice=>choiceWithinRecentMonths(choice,today,6))
        .filter(choice=>{
      ...
      const status = f.offerLoading || result.status === 'loading'
        ...
        : result.status === 'error'
          ...
          : result.status === 'ready' && visibleChoices.length <= 1
            ? '<div class="muted add-reward-status" data-nb-reviewed-message="empty">最近 6 个月暂无其他可直接选择的已审核奖励。</div>'
            : '';
      ...
      const intro=isCard
        ? '如果只是添加产品，直接点“添加到钱包”；如果还在完成开卡奖励，选择你申请时拿到的 Offer，我们会帮你追踪进度和截止日期。'
        : '如果只是添加产品，直接点“添加到钱包”；如果还在完成开户奖励，选择你开户时拿到的 Offer，我们会帮你追踪进度和截止日期。';
      view = {
        title: isCard ? '开卡奖励（可选）' : '开户奖励（可选）',
        body: `<div class="add-reward-flow">${selectedProductHeader(f.product,esc)}<div class="add-reward-intro"><span class="option-sub">${intro}</span></div>${status}<div class="option-list add-reward-list">${offerRows}<button class="option-row add-reward-row ${f.offer==='manual'?'selected':''}" type="button" data-action="add-offer-choice" data-id="manual"><span class="radio-dot"></span><span class="option-main"><span class="option-title">都不是，手动填写</span></span></button>${manualPanel}</div></div>`,
