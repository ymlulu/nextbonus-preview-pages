(() => {
  ...
  function followup(attempt,kind){
    ...
    if(!pending&&entry?.reconsiderationPhone) external.push(`<a class="nb-ah-btn secondary" href="tel:${esc(entry.reconsiderationPhone)}">拨打重新审核电话</a>`);
  ...
  function denied(attempt){
    ...
    const recon=entry?.reconsiderationPhone
      ? `<a class="nb-ah-btn secondary" href="tel:${esc(entry.reconsiderationPhone)}">拨打重新审核电话</a>`
      : '';
    const note=entry
  ...
  function success(productName){
    return `<div class="nb-ah-success"><strong>已添加到钱包</strong><p>${esc(productName)} 已保存；本次 Offer 的条件已经开始追踪。你现在看到的是新产品详情。</p></div>`;
  }
