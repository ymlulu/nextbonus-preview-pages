(() => {
  ...
  window.NextBonusPageRegistry.register('attention',function(ctx){
    ...
    return `<div class="content narrow attention-page"><div class="mock-page-top attention-top"><div><h1 class="mock-page-title">提醒</h1></div><select class="select mock-filter-select" id="attention-product-filter-local"><option value="all">全部产品</option>${productOptions.map(p=>`<option value="${esc(p.id)}" ${af===p.id?'selected':''}>${esc(p.label)}</option>`).join('')}</select></div>
