(() => {
  ...
  const { state, esc }=ctx;
    ...
    return `<div class="content narrow login-page"><div class="login-wrap"><div class="login-card mock-login-card"><div class="login-logo"><span class="nb-mini">NB</span></div><h1>登录 NextBonus</h1><p>继续查看你的关注、钱包和提醒。</p><div class="login-actions"><button class="btn primary login-primary" data-action="login-success">继续登录</button><button class="btn secondary" data-action="login-cancel">返回</button></div><div class="legal">登录后将回到你刚才想去的位置。</div></div></div></div>`;
