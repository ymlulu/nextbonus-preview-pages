  ...
  window.NextBonusPageRegistry.register('product-detail',function(ctx){
    ...
    const contactActions=creditCard&&creditCardView
      ...
      : (product.phone||product.loginUrl)
        ? `<div class="v4-pd-actions">${product.phone?`<button data-action="product-call" data-phone="${esc(product.phone)}">${icon('phone','nb-inline-action-icon nb-inline-action-icon--leading')}<span>客服电话</span></button>`:''}${product.phone&&product.loginUrl?'<i></i>':''}${product.loginUrl?`<button data-action="product-login-external" data-url="${esc(product.loginUrl)}">${icon('external-link','nb-inline-action-icon nb-inline-action-icon--leading')}<span>网上银行</span></button>`:''}</div>`
        : '';
    ...
    return `<div class="content v4-product-detail-page ${creditCard?'nb-card-detail':nonCredit?'nb-account-detail':''}">
      ...
      ${productSections}
      <section class="v4-pd-section v4-pd-history"><button class="v4-past-head" data-action="toggle-product-history"><span>历史记录 <b>${timeline.length}</b></span>${icon('chevron-down',`nb-disclosure-icon ${state.productHistoryOpen?'is-expanded':''}`)}</button>${state.productHistoryOpen?timelineFor(ctx,product,timeline):''}</section>
    </div>`;
  });
})();
