(() => {
  ...
  function productTile(ctx,product){
    ...
    return `<button class="v4-owned-product-card compact-tile v10-info-tile" data-action="open-product" data-id="${esc(product.id)}" aria-label="${esc(product.name)}"><span class="v10-info-logo" data-product-id="${esc(product.id)}">${logo?`<img src="${logo}" alt="" />`:`<span class="v10-logo-fallback">${esc(shortBrand(product.institution||product.name))}</span>`}</span><span class="v10-info-copy"><strong>${esc(primary)}</strong><small>${esc(secondary)}</small></span>${icon('chevron-right','v10-info-chevron nb-disclosure-icon')}</button>`;
  }

  function productSection(ctx,section){
    const {esc}=ctx;
    const {type,items,shown,expanded,collapsible,reliableDates,sortOpen,sortMode,sortLabel,className}=section;
    const sectionControls=collapsible
      ? `<div class="wallet-section-controls"><div class="product-sort-wrap"><button class="product-sort-button" data-action="toggle-product-sort" data-type="${esc(type)}" aria-expanded="${sortOpen?'true':'false'}">${esc(sortLabel)} ${icon('chevron-down','product-sort-chevron nb-disclosure-icon')}</button>${sortOpen?`<div class="product-sort-menu"><button class="${sortMode==='default'?'selected':''}" data-action="product-sort" data-type="${esc(type)}" data-value="default">默认顺序</button><button class="${sortMode==='recent'?'selected':''}" data-action="product-sort" data-type="${esc(type)}" data-value="recent">最近添加</button>${reliableDates?`<button class="${sortMode==='date-desc'?'selected':''}" data-action="product-sort" data-type="${esc(type)}" data-value="date-desc">最近开卡</button><button class="${sortMode==='date-asc'?'selected':''}" data-action="product-sort" data-type="${esc(type)}" data-value="date-asc">最早开卡</button>`:''}</div>`:''}</div><button class="product-section-toggle" data-action="toggle-product-section" data-type="${esc(type)}" aria-expanded="${expanded?'true':'false'}" aria-label="${expanded?'收起':'展开'}${esc(type)}">${icon('chevron-down',`product-section-chevron nb-disclosure-icon ${expanded?'is-expanded':''}`)}</button></div>`
      : '';
    return `<section class="v4-product-section-card v4-product-section ${className} ${collapsible&&!expanded?'is-collapsed':''}"><div class="v4-section-head"><h2>${esc(type)} <span class="section-count">${items.length}</span></h2>${sectionControls}</div>${expanded||!collapsible?`<div class="v4-owned-product-grid ${type==='信用卡'&&shown.length>1?'nb-wallet-card-stack':''}">${shown.map(product=>productTile(ctx,product)).join('')}</div>`:''}</section>`;
  }
  ...
  window.NextBonusPageRegistry.register('products',function(ctx){
    ...
    return `<div class="content products-page v4-products-page">
      <div class="v4-products-head"><div><h1>钱包</h1></div><button class="mock-add-btn v4-add-product" data-action="open-add-product" aria-label="添加产品"><svg class="v4-add-product-glyph" viewBox="0 0 24 24" aria-hidden="true"><path d="M12 5v14M5 12h14"/></svg><span class="v4-add-product-label">添加</span></button></div>
      <section class="v4-product-section-card v4-needs-attention"><div class="v4-section-head"><h2>待处理 <span class="attention-count-dot">${allActive.length}</span></h2><button class="mock-link" data-action="open-all-attention">查看全部 ${icon('chevron-right','nb-inline-action-icon')}</button></div>${top3.length?`<div class="v4-pp-attention-list">${top3.map(item=>attentionItem(ctx,item)).join('')}</div>`:`<div class="attention-zero-state">暂无待处理事项</div>`}</section>
      ...
      ${state.pastProducts.length?`<section class="v4-past-products ${state.pastOpen?'is-open':''}"><button class="v4-past-head" type="button" data-action="wallet-toggle-past" aria-expanded="${state.pastOpen?'true':'false'}"><span class="v4-past-head-title"><strong data-nb-type="headline" data-nb-emphasis="true">历史产品</strong><small>${state.pastProducts.length}</small></span>${icon('chevron-down',`v4-past-head-chevron nb-disclosure-icon ${state.pastOpen?'is-expanded':''}`)}</button>${state.pastOpen?`<div class="v4-past-list">${state.pastProducts.map(product=>`<button class="v4-past-row" type="button" data-action="open-product" data-id="${esc(product.id)}"><strong class="v4-past-row-title" data-nb-type="subheadline" data-nb-emphasis="true">${esc(product.name)} ${esc(product.instance||'')}</strong><small class="v4-past-row-status">${esc(product.statusText||product.status)}</small>${icon('chevron-right','v4-past-row-chevron nb-disclosure-icon')}</button>`).join('')}</div>`:''}</section>`:''}
    </div>`;
  });
})();
