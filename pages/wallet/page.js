(() => {
  ...
  window.NextBonusPageRegistry.register('products',function(ctx){
    ...
    return `<div class="content products-page v4-products-page">
      ...
      ${state.pastProducts.length?`<section class="v4-past-products ${state.pastOpen?'is-open':''}"><button class="v4-past-head" type="button" data-action="wallet-toggle-past" aria-expanded="${state.pastOpen?'true':'false'}"><span class="v4-past-head-title"><strong>历史产品</strong><small>${state.pastProducts.length}</small></span><span class="v4-past-head-chevron" aria-hidden="true">›</span></button>${state.pastOpen?`<div class="v4-past-list">${state.pastProducts.map(product=>`<button class="v4-past-row" type="button" data-action="open-product" data-id="${esc(product.id)}"><strong class="v4-past-row-title">${esc(product.name)} ${esc(product.instance||'')}</strong><small class="v4-past-row-status">${esc(product.statusText||product.status)}</small><span class="v4-past-row-chevron" aria-hidden="true">›</span></button>`).join('')}</div>`:''}</section>`:''}
    </div>`;
  });
})();
