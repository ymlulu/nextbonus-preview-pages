(() => {
  ...
  function section(title,items,cls,esc){
    if(!items.length&&cls==='in-progress')return '';
    if(!items.length){
      return `<section class="nb-watchlist-section ${cls}">
        <div class="nb-watchlist-section-head"><div><h2>${esc(title)}</h2></div><span class="nb-watchlist-count">0</span></div>
        <div class="nb-watchlist-section-empty">暂时没有内容</div>
      </section>`;
    }
    return `<section class="nb-watchlist-section ${cls}">
      <div class="nb-watchlist-section-head"><div><h2>${esc(title)}</h2></div><span class="nb-watchlist-count">${items.length}</span></div>
  ...
  window.NextBonusPageRegistry.register('wishlist',function(ctx){
    ...
    return `<div class="content wishlist-page nb-watchlist-page">
      <div class="nb-watchlist-header"><div><h1>关注</h1></div></div>
      ...
      `${section('进行中',inProgress,'in-progress',ctx.esc)}
       ${section('已关注',saved,'saved',ctx.esc)}
