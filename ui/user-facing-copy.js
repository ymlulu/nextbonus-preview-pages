(() => {
  ...
  const EXACT = new Map([
    ...
    ['这次申请已记录为未通过。你可以选择尝试 Reconsideration；这里只显示已经核验的后续信息。', '这次申请已经记录为未通过。如果银行支持，你可以尝试联系人工重新审核。'],
    ['部分银行的拒绝结果可能可以尝试 Reconsideration；目前没有已核验的具体入口。', '部分银行可以在被拒后联系人工重新审核，目前还没有确认可靠的联系方式。']
  ]);

  const PARTIAL = [
    [/申请上下文已经保存，之后回来不需要重新选择产品或 Offer。/g, '这次申请信息已经保存，之后回来可以直接继续。'],
    [/NextBonus 已保留这次申请上下文，你可以随时回来更新结果。/g, '这次申请已经帮你记下来了，你可以随时回来更新结果。'],
    [/点击“直接申请”只冻结本次 Product \/ Offer \/ Assessment 上下文，并不等于你已经提交申请。只有你确认结果后，NextBonus 才会创建实际产品或后续状态。/g, '点击“直接申请”只会帮你记住这次申请的产品和奖励，不代表你已经提交申请。等你回来确认结果后，再决定是否加入“我的”并开始追踪奖励。'],
    [/已读取 Application Follow-up Registry 中已核验的[^。]*后续信息。/g, '下面是已经确认过的申请状态和后续处理方式。'],
    [/目前还没有已核验的状态查询 \/ 后续处理信息，因此 Preview 不展示未经核验的电话或建议。你仍可以保存当前状态，之后再更新结果。/g, '目前还没有确认可靠的状态查询或后续处理方式。你可以先保存当前状态，之后再回来更新。'],
    [/“直接申请”本身不要求登录；但创建 UserProduct、奖励追踪和任务属于个人数据，需要登录后保存。/g, '登录后才能把这张卡加入“我的”，并保存奖励进度和提醒。'],
    ...
    [/已保存；本次 Offer 的条件已经开始追踪。你现在看到的是新产品详情。/g, '已保存，相关奖励条件也已经加入提醒。'],
